...
genann *genann_init(int inputs, int hidden_layers, int hidden, int outputs) {
    ...
    ret->prev_output = ret->delta + (ret->total_neurons - ret->inputs); // Initialize prev_output

    /* Initialize prev_output to zero. */
    memset(ret->prev_output, 0, sizeof(double) * outputs);
...
genann *genann_read(FILE *in) {
    ...
    for (i = 0; i < ann->outputs; ++i) {
        errno = 0;
        rc = fscanf(in, " %le", ann->prev_output + i);
        if (rc < 1 || errno != 0) {
            perror("fscanf");
            genann_free(ann);
            return NULL;
...
genann *genann_copy(genann const *ann) {
    ...
    ret->output = ret->weight + ret->total_weights;
    ret->delta = ret->output + ret->total_neurons;
    ret->prev_output = ret->delta + (ret->total_neurons - ret->inputs);
...
double const *genann_run(genann const *ann, double const *inputs) {
    ...
    /* Append the previous output to the input. */
    memcpy(ann->output + ann->inputs, ann->prev_output, sizeof(double) * ann->outputs);
    ...
    if (!ann->hidden_layers) {
        ...
        for (j = 0; j < ann->outputs; ++j) {
            double sum = *w++ * -1.0;
            for (k = 0; k < ann->inputs + ann->outputs; ++k) {
                sum += *w++ * i[k];
            }
        ...
        /* Update prev_output. */
        memcpy(ann->prev_output, ret, sizeof(double) * ann->outputs);
...
void genann_train(genann const *ann, double const *inputs, double const *desired_outputs, double learning_rate) {
    ...
    /* Calculate the error for the hidden layers, starting from the last hidden layer. */
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        /* Find first output and delta in this layer. */
        double const *o = ann->output + ann->inputs + (h * ann->hidden);
        double *d = ann->delta + (h * ann->hidden);

        /* Find first delta in the following layer (which may be hidden or output). */
        double const * const dd = ann->delta + ((h + 1) * ann->hidden);

        /* Find first weight in the following layer (which may be hidden or output). */
        ...
        for (j = 0; j < ann->hidden; ++j) {
            ...
            /* Compute the delta for this neuron. */
            *d = *o * (1.0 - *o) * delta;
            ++d; ++o;
        }
    }
    ...
    /* Update the weights for the output layer. */
    {
        ...
        /* Find first output in the previous layer. */
        ...
        for (j = 0; j < ann->outputs; ++j) {
            *w++ += *d * learning_rate * -1.0; /* Bias weight. */
            for (k = 1; k < (ann->hidden_layers ? ann->hidden : ann->inputs + ann->outputs) + 1; ++k) {
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        for (j = 0; j < ann->hidden; ++j) {
            *w++ += *d * learning_rate * -1.0; /* Bias weight. */
            for (k = 1; k < (h == 0 ? ann->inputs + ann->outputs : ann->hidden) + 1; ++k) {
                *w++ += *d * learning_rate * i[k - 1];
            }
            ++d;
        }
...
void genann_write(genann const *ann, FILE *out) {
    ...
    /* Write prev_output. */
    for (i = 0; i < ann->outputs; ++i) {
        fprintf(out, " %.20e", ann->prev_output[i]);
    }
}
